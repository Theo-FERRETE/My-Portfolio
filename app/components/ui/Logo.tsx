...
export default function Logo({ size = 'md' }: { size?: 'md' | 'lg' }) {
  ...
  const name = size === 'lg' ? 'text-xl sm:text-2xl' : 'text-sm sm:text-base';

  return (
    <span className="inline-flex items-center gap-2.5">
      <span
        className={`inline-flex items-center justify-center rounded-lg bg-accent text-background font-bold tracking-tight ${mark}`}
        aria-hidden
      >
        TF
      </span>
