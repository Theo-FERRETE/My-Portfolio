  ...
  return (
    <section ref={ref} className="py-14 sm:py-16 bg-surface/40 border-y border-border">
      <div className={`container mx-auto px-4 sm:px-6 reveal ${inView ? 'reveal-in' : ''}`}>
        ...
        <ul className="max-w-6xl mx-auto mt-8 sm:mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
          {REASONS.map((reason) => (
            <li key={reason.title} className="glass-card rounded-xl p-5 sm:p-6">
              <span className="inline-flex w-10 h-10 items-center justify-center rounded-lg bg-accent/15 text-accent">
                <reason.icon size={20} aria-hidden />
              </span>
              <h3 className="mt-4 text-lg font-bold text-foreground">{reason.title}</h3>
